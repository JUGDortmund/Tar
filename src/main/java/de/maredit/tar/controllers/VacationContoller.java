...
public class VacationContoller extends WebMvcConfigurerAdapter {
	...
	private static final Logger LOG = LogManager
			.getLogger(VacationContoller.class);
	...
	public String index(HttpServletRequest request, Model model,
		...
		List<Vacation> vacations = this.vacationRepository
				.findVacationByUserAndStateNotOrderByFromAsc(user,
						State.CANCELED);
	...
	public String substitution(@RequestParam(value = "id") String id,
		...
		vacation.setState((approve) ? State.WAITING_FOR_APPROVEMENT
				: State.REJECTED);
		this.vacationRepository.save(vacation);

		MailObject mail = (approve ? new SubstitutionApprovedMail(vacation)
				: new SubstitutionRejectedMail(vacation));
	...
	public String approval(@RequestParam(value = "id") String id,
		...
		MailObject mail = (approve ? new VacationApprovedMail(vacation)
				: new VacationDeclinedMail(vacation));
		this.mailService.sendMail(mail);
	...
	public String saveVacation(@Valid Vacation vacation,
			BindingResult bindingResult, Model model) {
		if (bindingResult.hasErrors()) {
			bindingResult.getFieldErrors().forEach(
					fieldError -> LOG.error(fieldError.getField() + " "
							+ fieldError.getDefaultMessage()));
			User selectedUser = this.userRepository.findByUidNumber(vacation
					.getUser().getUidNumber());
			List<User> users = getSortedUserList();
			List<Vacation> vacations = this.vacationRepository
					.findVacationByUserAndStateNotOrderByFromAsc(selectedUser,
							State.CANCELED);
			List<User> managerList = getManagerList();
			List<Vacation> substitutes = this.vacationRepository
					.findVacationBySubstitute(applicationController
			...
			setVacationFormModelValues(model, selectedUser, users, vacations,
					managerList, substitutes, substitutesForApproval, approvals);
			return "application/index";
		} else {
			...
			this.vacationRepository.save(vacation);
			this.mailService.sendMail(newVacation ? new VacationCreateMail(
					vacation) : new VacationModifiedMail(vacation));
			return "redirect:/";
		}
	}
	...
	public String cancelVacation(HttpServletRequest request,
	...
	private void setVacationFormModelValues(Model model, User selectedUser,
			List<User> users, List<Vacation> vacations, List<User> managerList,
			List<Vacation> substitutes, List<Vacation> substitutesForApproval,
	...
	private List<User> getManagerList() {
		...
		try {
			managerList = userRepository.findByUsernames(ldapService
					.getLdapSupervisorList());
			managerList = managerList
					.stream()
					.filter(e -> e.isActive())
					.sorted((e1, e2) -> e1.getLastname().compareTo(
							e2.getLastname())).collect(Collectors.toList());
	...
	private User getUser(HttpServletRequest request) {
		...
		if (selected == null) {
			...
			user = this.userRepository.findUserByUsername(String
					.valueOf(selected));
		}
