...
public class UserSyncTask {
	...
	public void syncLdapUser() {
		try {
			...
			for (User user : ldapUserList) {
				User localUser = userRepository.findByUidNumber(user
						.getUidNumber());
				if (localUser == null) {
					localUser = user;
					LOG.debug("User created. username: {} / uidNumber: {}",
							user.getUsername(), user.getUidNumber());
				} else {
					updateUser(localUser, user);
					LOG.debug("User updated. username: {} / uidNumber: {}",
							user.getUsername(), user.getUidNumber());
				}
	...
	private void updateUser(User resultEntry, User user) {
		...
		try {
			user.setPhoto(resultEntry.getPhoto().getBytes("UTF8"));
